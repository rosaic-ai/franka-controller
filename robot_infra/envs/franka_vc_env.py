...
class FrankaVC(gym.Env):
    ...
    def get_robot_state(self):
        ...
        response = requests.post(self.url + 'getstate')
        state = response.json()
        return state
    ...
    def quat_2_euler(self, quat):
        """
        쿼터니언을 받아서 오일러 각(roll, pitch, yaw)으로 변환하고, 그 결과를 도 단위로 반환합니다.
        """
        # 쿼터니언을 Rotation 객체로 변환
        rotation = R.from_quat(quat)
        # 오일러 각으로 변환, 'xyz' 순서에 따라 roll, pitch, yaw 반환
        euler_angles = rotation.as_euler('xyz', degrees=True)  # 각을 바로 도 단위로 변환
        return euler_angles  # (roll, pitch, yaw) 순서로 반환됨

    def euler_2_quat(self, roll=0, pitch=0, yaw=0):
        """
        오일러 각 (yaw, pitch, roll)을 쿼터니언으로 변환합니다.
        모든 입력 각은 라디안 단위로 제공되어야 합니다.
        """
        roll = roll +180
        rotation = R.from_euler('xyz', [roll, pitch, yaw], degrees=True)
        quat = rotation.as_quat()  # x, y, z, w 순서로 반환
        return quat
